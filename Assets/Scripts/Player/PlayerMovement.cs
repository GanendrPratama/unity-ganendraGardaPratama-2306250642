...
public class PlayerMovement : MonoBehaviour
{
    ...
    public void Move()
    {
        ...
        rb.linearVelocity += moveVelocity * Time.deltaTime * moveDirection;

        // Limit the player's velocity so that it will only reach maxSpeed and not further.
        rb.linearVelocity = new(Mathf.Clamp(rb.linearVelocity.x, -maxSpeed.x, maxSpeed.x), Mathf.Clamp(rb.linearVelocity.y, -maxSpeed.y, maxSpeed.y));
        ...
        rb.linearVelocity -= GetFriction() * Time.deltaTime;
        ...
        if (Math.Abs(rb.linearVelocity.x) < stopClamp.x && moveDirection.x == 0 || moveDirection.x > -0 && ppos.x >= 0.99f || moveDirection.x < 0 && ppos.x <= 0.01f)
            rb.linearVelocity = new(0, rb.linearVelocity.y);

        if (Math.Abs(rb.linearVelocity.y) < stopClamp.y && moveDirection.y == 0 || moveDirection.y > -0 && ppos.y >= 0.95f || moveDirection.y < 0 && ppos.y <= -0.01f)
    ...
    public Vector2 GetFriction()
    {
        ...
        if (moveDirection.y > 0)
            ...
            totalFriction.y = -moveFriction.y;
        else if (moveDirection.y == 0 && rb.linearVelocity.y > 0)
